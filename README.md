...
控制台每秒刷新 Livis 连接状态、当前模型与推理等级。每个活跃线程会展示任务输入、已运行时长、当前阶段、Codex 提供的推理摘要、Skill 和工具调用轨迹；不展示模型内部隐藏思维链。后台任务以“状态、任务 ID、节点、更新时间、结果”表格分页展示，每页 10 条、最多 50 条，不长期保存执行轨迹。
